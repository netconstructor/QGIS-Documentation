...
\subsection{Extension de saisie de coordonnées}
...
L'extension saisie de coordonnées, facile à utiliser, permet 
d'afficher des coordonnées sur la carte pour deux systèmes de 
coordonnées de référence (CRS) sélectionnés. Vous pouvez cliquer 
sur un point et copier les coordonnées dans le presse-papier, ou 
...
\begin{enumerate}
  ...
  \mainmenuopt{Préférences} et cliquez sur l'onglet \tab{Système de coordonnées de préférence (CRS)}. 
  ...
  de coordonnées projeté \filename{"NAD27/Alaska Albers"} dont l'EPSG est 2964 (voir aussi 
  ...
  \item Chargez l'extension Saisie de Coordonnées dans le Gestionnaire d'extensions (voir la Section
  \ref{sec:load_core_plugin}) et cliquez sur l'icône \toolbtntwo{coordinate_capture}{Saisie de coordonnées}.
  ...
  des coordonnées} et sélectionnez le Système de Coordonnées Géographiques \filename{WGS84} (EPSG 4326).
  \item Vous pouvez désormais cliquer n'importe où dans la fenêtre carte et les coordonnées des points 
  sélectionnés s'afficheront en \filename{"NAD27/Alaska Albers"} et \filename{WGS84}, comme indiqué dans la
  Figure \ref{fig:coordinate_capture_dialog}.
  \item Pour activer le suivi des coordonnées du curseur de la souris, cliquez sur l'icône \toolbtntwo{tracking}{suivi du curseur}.
  \item Vous pouvez également copier les coordonnées vers le presse-papiers.
\end{enumerate}
