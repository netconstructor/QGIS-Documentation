...
\subsection{Charger des données GPS à partir d'un fichier}\label{label_loadgps}
...
% \usertext{\%type} will be replaced by {}\og \usertext{-w}\fg if you are 
% downloading waypoints, {}\og \usertext{-r}\fg if you are downloading routes and
% {}\og \usertext{-t}\fg if you are downloading tracks.
\usertext{\%type} sera remplacé par {}\og \usertext{-w}\fg  si vous téléchargez des waypoints, {}\og \usertext{-r}\fg si vous téléchargez des routes et
{}\og \usertext{-t}\fg si vous téléchargez des tracks.
...
% {}\og \usertext{gpsbabel\%type -i garmin -o gpx \%in \%out}\fg (this is actually
...
% {}\og \usertext{/dev/ttyS0}\fg to the file {}\og \usertext{output.gpx}'', \qg will
% replace the keywords and run the command {}\og \usertext{gpsbabel -w -i garmin
% -o gpx /dev/ttyS0 output.gpx}''.
Si vous créez un type de périphérique avec la commande de téléchargement {}\og \usertext{gpsbabel\%type -i garmin -o gpx \%in \%out}\fg (c'est actuellement
...
les waypoints à partir du port {}\og \usertext{/dev/ttyS0}\fg  vers le fichier {}\og \usertext{output.gpx}'', \qg remplacera les mots-clés et lancera la
commande {}\og \usertext{gpsbabel -w -i garmin -o gpx /dev/ttyS0 output.gpx}\fg .
