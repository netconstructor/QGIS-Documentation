...
QGIS se base sur vos paramtrès globaux pour définir la langue de l'interface. Si vous voulez en changer vous devez le spécifier en saissisant un code. 
...
Démarrer QGIS avec un projet existant est possible, il sufit de rajouter cette option suivi du nom de votre projet et QGIS se lancera avec toutes les couches de ce fichier.
...
Pour démarrer avec une étendue cartographique spécifique utilisez cette option. Vous devez ajouter les limites de votre étendue dans l'ordre suivant en les éparant par une virgule :
\begin{verbatim}
--extent xmin,ymin,xmax,ymax
\end{verbatim}
...
\textbf{Note:} Les décorations de fenêtre peuvent vous apparaître différement sur votre système en fonction de votre système d'exploitation et de votre gestionnaire de fenêtre.
...
La barre de menu fournit un accès aux différentes fonctionnalités de QGIS par le bias de menus hiérarchiques. Le menu supérieur et un résumé de certaines options sont listés ci-dessous, avec les icônes des outils correspondants dans la barre d'outils et leurs raccourcis clavier. Bien que les options de menu est des outils qui leur correspondent et vice-versa, les menus ne sont pas organisé comme les barres d'outils. La barre contenant l'outil est affiché à la suite de chaque option de menu. Pour plus d'informations sur les outils et les barres d'outils, veuillez lire la section \ref{label_toolbars}.
