...
Working with GRASS vector-data is described in chapter \ref{sec:grass}.
...
toolbar button\index{shapefile!loading}. This same tool can be used to
...
\begin{figure}[ht]
   \begin{center}
   \caption{QGIS with vmap0 Shapefile of Spain loaded}\label{fig:loadedshapefile}\smallskip
...
\includegraphics[width=0.7cm]{addpostgis} The first time
...
\includegraphics[width=0.7cm]{addpostgis} Once you have one or more
...
geometry\_columns table. From version \OLD on, QGIS can load layers that do not have
...
\qgistip{Like the import-tool \textsl{shp2pgsql} there is also a tool to export
PostGIS-datasets into shapefiles: \textsl{pgsql2shp}. This is shipped within your
...
\minisec{ogr2ogr}
Beside \textit{shp2pgsql} and \textit{SPIT} there is another tool for feeding
geodata in PostGIS: \textit{ogr2ogr}. This is part of your GDAL installation.
...
This will import the shapefile alaska.shp into the PostGIS-database \textit{postgis}
using the user \textit{postgres} with the password \textit{topsecret} on host
\textit{myhost.de}.

Note that OGR must be built against PostgreSQL to support PostGIS.
...
documentation available at http://postgis.refractions.net}
index is:
...
\subsubsection{Vector transparency} \label{sec:vect_transparency} \index{vector layers!transparency}
...
This is very useful for displaying several vector-layers.
...
Layer editing is most versatile when used on
...
before you start editing, and also at regular intervals during editing.

QGIS is still at a pre-version 1.0 stage, and so may not be
able to protect your data in all situations.
}
\end{Tip}
...
\index{Start Editing}
\includegraphics[width=0.4cm, clip=true]{StartEditing} \textit{Start editing} or
\index{Stop Editing}
\includegraphics[width=0.4cm, clip=true]{StopEditing}  \textit{Stop editing}
buttons from the toolbar to toggle the editing mode.\index{editing!icons}

\begin{Tip}[h]\caption{\textsc{Editing a Map is Different to Editing an Attribute Table}}
\qgistip{In this version of QGIS, the \textit{Start editing}/\textit{Stop editing}
couplet on the map view acts separately to the \textit{Start Editing}/\textit{Stop Editing}
couplet on the attribute table.
}
\end{Tip}

\begin{Tip}[h]\caption{\textsc{Save Regularly}}
\qgistip{Remember to toggle \textit{Allow editing} off or select the \textit{Stop editing}
button regularly.  This allows you to save your changes thus far,
...
zoom in to your area of interest first.  This 
...
\begin{itemize}
\item Add Features
\item Add Vertices of a Feature
\item Move Vertices of a Feature
\item Delete Vertices of a Feature
...
or \textit{Capture Polygon} icon
on the toolbar to put the QGIS cursor
...
dialog box does not check that the
data entered matches the type expected (e.g. numeric vs. text),

Make sure of this before
pressing \textit{Ok}, otherwise you may find the error
...
You can add new verticies to a feature by
using the \textit{Add Vertex} icon
on the toolbar.
...
You can delete new verticies to a feature by
using the \textit{Delete Vertex} icon
on the toolbar.
...
To delete a feature, select it using the selection tool
and choose \textit{Delete selection} from the editing tools.

\begin{Tip}[h]\caption{\textsc{Feature Deletion Support}}
\qgistip{When editing ESRI shapefiles, the deletion
of features only works if QGIS is linked to a GDAL version 1.3.2 or greater. The OS X and Windows versions of QGIS available from the download site are built using GDAL 1.3.2 or higher.
}
\end{Tip}
...
\minisec{Snap Mode}
\index{editing!snap}
QGIS allows digitised vertices to be snapped to other vertices of the same layer. To 
...
contained in the highlighted field by clicking on the \textit{Sample} button\index{Query
