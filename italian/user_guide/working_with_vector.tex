...
Per importare uno shapefile, cliccare sull'icona \toolbtntwo{spiticon}{Importa
shapefile in PostgreSQL} nella barra degli strumenti per aprire la finestra di
dialogo \dialog{SPIT - Utilità di importazione ESRI Shapefile in PostGIS}.
Selezionare il database PostGIS al quale si desidera connettersie cliccare su
\button{Connetti}. Ora è possibile aggiungere uno o più files alla coda
...
elaborato.
\begin{Tip}\caption{\textsc{Imporare shapefiles contenenti parole riservate a PostgreSQL}}\index{PostGIS!SPIT!reserved words}
...
\begin{description}
 ...
 forniti è possibile scegliere \selectstring{Stile di riempimento}{? Texture}
 e cliccare su \browsebutton per selezionare un retino personalizzato.
 Attualmente sono
 supportati i formati \filename{*.jpeg, *.xpm, and *.png}.
\end{description}
...
Si noti che la liguetta \tab{Etichette} fornisce una \classname{Anteprima}
...
La finestra di dialogo \dialogo{Risultati individuati} che compare quando si
