...
\index{shapefiles}\index{ESRI!shapefiles}\index{files SHP}, il formato di
interscambio MapInfo MIF\index{files MIF}\index{MapInfo!files MIF}
e il formato nativo MapInfo TAB (native format).\index{files TAB}\index{MapInfo!files TAB}
...
ulteriori plugins specifici.\index{testo delimitato}
...
\index{layer vettoriali!ESRI shapefiles}
\index{shapefiles}
...
un indice spaziale. Un \index{indice spaziale!shapefiles} indice spaziale
...
\index{layer vettoriali!ArcInfo Coverage}
...
Per usare livelli PostGIS layers bisogna:\index{PostgreSQL!caricamento layers}
...
cliccando sul pulsante \button{Prova connessione} \index{PostgreSQL!connessione!testing}.
...
\subsubsection{Caricare un layer PostGIS}\index{PostgreSQL!caricamento layers}
...
\index{PostgreSQL!dettagli strato}
...
c'è anche lo strumento per l'esportazione di dati PostGIS come shapefiles:
...
elaborato.
\begin{Tip}\caption{\textsc{Importare shapefiles contenenti parole riservate a PostgreSQL}}\index{PostGIS!SPIT!parole riservate}
...
\subsubsection{Linguetta Azioni}\index{azioni}\label{label_actions}
...
\subsubsection{La linguetta Attributi}\index{attributi}\label{label_attributes}
...
\index{modifica!un layer esistente}
...
\index{editing!tagliare elementi}
\index{editing!copiare elementi}
\index{editing!incollare elementi}
...
\index{editing!salvare modifiche}
...
\subsubsection{Creare un nuovo layer}\label{sec:create shape}\index{editing!creare un nuovo layer}
...
Builder!generare un campione dei dati}. In questo modo vengono richiamati i primi 25
valori distinti dai campi del database. Per avere una lista completa di tutti
i valori di un campo, cliccare sul pulsante \button{All} \index{Query Builder!ottenere tutti
i valori}. Per aggiungere un campo o un valore selezionato alla query, fare
doppio-click su di esso\index{Query Builder!aggiungere campi}. È possibile usare
...
\index{Query Builder!testare interrogazioni}. Se l'espressione è correta, verrà
...
Builder!cambiare la definizione di uno strato}
