...
das ESRI Shape,\index{shapefiles}\index{ESRI!shapefiles}\index{SHP files}
MapInfo MIF (Austauschformat)\index{MIF files}\index{MapInfo!MIF files}
und MapInfo TAB (Natives Format).\index{TAB files}\index{MapInfo!TAB files}
...
erstellt werden. Ein\index{spatial index!shapefiles} räumlicher Index erhöht
die Geschwindigkeit beim Zoomen und Verschieben. Räumliche Indizes haben in
QGIS die Endung \filename{.qix}.
...
Schritte:\index{PostgreSQL!loading layers}
...
\hline Benutzername \index{PostgreSQL!username}& Benutzername, um sich bei
...
drücken\index{PostgreSQL!connection!testing}. 
...
\begin{Tip}
\caption{\textsc{PostGIS-Layer exportieren}\index{PostGIS!Exporting}}
...
angezeigt\index{PostGIS!SPIT!loading}.
...
für alle Ebenen in PostgreSQL ein räumlicher Index\index{PostGIS!spatial
...
The Reiter \tab{Allgemein} im Dialog \dialog{Layereigenschaften} ermöglicht
...
Layers zugewiesen.\index{vector layers!renderers!single symbol}
...
Darstellung zugewiesen.\index{vector layers!renderers!graduated symbol}
\item[Fortlaufende Farbe] - Objekten eines Layers wird anhand von definierten,
numerischen Werten eine fortlaufende Farbe zugewiesen.\index{vector
layers!renderers!continuous color}
\item[Eindeutiger Wert] - eine definierte Darstellung wird jeder Objektklasse
eines Layers anhand einer definierten Attributspalte zugewiesen.\index{vector
layers!renderers!unique value}
\end{description}

\minisec{Stiloption} \label{sec:style_options} \index{vector
layers!styles}
...
\begin{description}
 \item[Umrandunsstil] - Definition verschiedener Linienarten und auch keine
...
Umrandunglinien von Objekten
 ...
 \item[Fullstil] - Definition des Füllstils von Polygonen. Neben Ausgefüllt
...
\minisec{Transparenz}\label{sec:vect_transparency} \index{vector
layers!transparency}
...
angezeigt und können ausgewählt und benutzt werden. Sie können auf eine
...
Sie z.B. einen Punktlayer mit einzelne Punkten haben, an denen Photos geschossen
wurden, dann könnten Sie eine Aktion erstellen, über die sie dann das
entsprechende Photo anzeigen lassen können, wenn Sie auf den Punkt in der
Karte klicken. Man könnte auch zu bestimmten Attributen webbasierte
...
\subsubsection{Attribute}\index{attributes}\label{label_attributes}
...
\subsection{Editierfunktionen}\index{editing}
...
\textbf{Bemerkung} - die Vorgehensweise zum Editieren GRASS Vektorlayern ist
anders und wird in Kapitel~\ref{grass_digitising} beschrieben.
...
wichtig die Fangtoleranz und den Suchradius für Stützpunkte (Vertice)
festzulegen, um optimale Ergebnisse zu erzielen. 

\minisec{Fangtoleranz}

Die Fangtoleranz ist der Abstand des QGIS benutzt, um beim digitalisieren den
...
Werkzeugleiste aktiviert sein. Alternative können Sie dazu auch mit der
...
Das Verschieben des Layers im Kartenfensters während des Digitalisierens ist
...
\item \toolbtntwo{mActionMoveFeature}{Objekte verschiebens}
...
zuweisen. Das neue Inselpolygon muss ausserhalb des selektierten Multipolygon
...
(Quelle)
\item Laden oder erstellen einen Layer, in den die kopierten Objekte
...
\subsubsection{Neue Vektorlayer erstellen}\label{sec:create
shape}\index{editing!creating a new layer}
...
\button{Stichprobe}\index{Query Builder!generating sample list} klicken.
...
\button{Alle}\index{Query Builder!getting all values}. Um eine Attributspalte
in das untere Fenster (SQL where clause) zu kopieren, doppelklicken sie auf
den Namen in der Box (Felder)\index{Query Builder!adding fields}. Um eine
Abfrage zu erstellen, können Sie die verschiedenen Operatoren benutzen.

Um eine Abfrage zu testen, klicken Sie auf den Knopf
\button{Test}\index{Query Builder!testing queries}. Dies erzeugt ein Fenster,
...
\subsection{Selektion durch Abfrage}\label{sec:select_by_query}
\index{PostgreSQL!query builder}
\index{PostGIS!query builder}
\index{query builder!PostgreSQL}
\index{query builder!PostGIS}

QGIS ermöglicht es, Objekte über den Query Builder abzufragen, ähnlich wie in
Abschnitt~\ref{sec:query_builder} vorgestellt. Im oberen Beispiel haben wir
den Query Builder dazu verwendet, um die Abfrage eines PostGIS-Layers als
'virtuellen Layer' anzuzeigen. Dabei werden alle abgefragten Objekte, welche
die Abfragekriterien erfüllen, farblich hervorgehoben, Dies funktioniert
grundsätzlich auch mit allen anderen, durch QGIS unterstützen Vektorformaten. 

Um eine geladene OGR- oder GRASS-Ebene abzufragen, öffnen Sie jedoch nichg
auf den \dialog{Layereigenschaften} Dialog, sondern klicken auf den Knopf
\toolbtntwo{mActionOpenTable}{Attributtabelle öffnen} in der
Werkzeugleiste und dann Dialogfenster auf den Knopf \button{Erweitert}.
Dadurch wird auch der Query Builder gestartet und ermöglicht Abfragen wie in
Abschnitt \ref{sec:query_builder} beschrieben.
