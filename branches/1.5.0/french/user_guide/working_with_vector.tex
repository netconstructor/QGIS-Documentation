...
Si vous chargez un shapefile avec un fichier \filename{.prj} et \qg ne peut en déterminer le système de cordonnées employé, vous devrez définir manuellement le système via l'onglet \tab{Général} des \dialog{Propriétés de la couche}. Cela est dû au fait que souvent les fichiers \filename{.prj} ne contienntnent pas la totalité des paramètres tel qu'utilisés dans \qg et listé dans la fenêtre des \dialog{SCR}.
...
vector layers.
\qg permet de définir une transparence pour chaque couche vecteur. Ceci peut-être fait avec le curseur \slider{Transparence} de l'onglet \tab{Convention des signes} (voir fig. \ref{fig:vector_symbology}). Ceci est très utile pour superposer plusieurs couches vecteur.
