...
\subsection{Couches vecteurs dépassants les \degrees{180} de longitude}
...
\minisec{Transparence d'une couche vectorielle} \label{sec:vect_transparency} \index{couches vecteur!transparence}
...
%vector layers.
\qg permet de définir une transparence pour chaque couche vecteur. Ceci peut-être fait avec le curseur \slider{Transparence} de l'onglet \tab{Convention des signes} (voir fig. \ref{fig:vector_symbology}). Ceci est très utile pour superposer plusieurs couches vecteur sur un même canevas.
...
Il vous faut d'abord activer la nouvelle symobologie en cliquant sur le bouton \button{Nouvelle symbologie} dans les \dialog{Propriétés de la couche}. L'interface vous propose alors de choisir entre 3 rendus : symbole unique, catégorisé ou gradué. Selon le moteur de rendu choisi, cette fenêtre proposera des options différentes.
...
Le moteur de rendu pour symbole unique est utilisé pour représenter toutes les entités de la couche avec un seul symbole défini par l'utilisateur. Les propriétés, qui peuvent être ajustées dans la fenêtre de symbologie, dépendent du type de géométrie de la couche, mais partagent une structure similaire. En haut à gauche figure un aperçu du symbole tel qu'il apparaîtra. En bas est affichée la liste des symboles déjà existants dans le style courant, prêts à être sélectionné d'un simple clic. Le symbole courant peut être modifié en utilisant le bouton \button{Propriétés} qui ouvre une fenêtre \dialog{Propriétés du symbole}, ou en utilisant le bouton \button{}
...
\subsection{Nouvel Étiquetage}\index{New labeling}\label{newlabel}
...
\minisec{Ajouter des entités} \index{couches vecteur!déplacer!entités}
...
\minisec{Opérations basiques}\index{vector layers!Node Tool}
...
\index{couches vecteurs!ajouter!partie}
...
%want to split.
Vous pouvez diviser une entité en utilisant le bouton \toolbtntwo{mActionSplitFeatures}{Couper Entités} situés dans la barre de numérisation. Pour pcouper, dessinez une ligne en travers de l'entité avec cet outil.
...
\index{couches vecteurs!rotation!symbole}
...
\subsection{Créer de nouvelles couches Shapefile et Spatialite}\label{sec:create shape}\index{editing!creating a new shape layer}
...
\minisec{Créer une nouvelle couche SpatiaLite}\label{sec:create spatialite}\index{editing!creating a new spatialite layer}
