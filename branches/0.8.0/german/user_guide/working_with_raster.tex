...
räumlich korrekt mit anderen Vektorebenen oder Rasterdaten überlagert werden
...
auf das \textsl{Füge eine Rasterebene hinzu} Icon oder über die Menüleiste unter
\textsl{Ebene~->~Füge eine Rasterebene hinzu} geladen werden. Mehrere Raster
...
mit der rechten Maustaste auf den Ebenennamen in der Kartenlegende. Dadurch
erscheint ein Popupmenue für Rasterebenen und bietet eine Vielzahl von
Optionen:\index{raster layers!context menu}

\begin{figure}[ht]
   \begin{center}
   \caption{Popupmenü für Rasterebenen}\label{fig:raster_contextmenu}\smallskip
...
\begin{itemize}
\item Eigenschaften
\item Auf die Ebenenausdehnung zoomen
\item Zur Übersicht hinzufügen
\item Aus Übersicht entfernen
\item Entfernen
\item Umbenennen 
...
Wähle \textsl{Eigenschaften} aus dem Popupmenü, um die
Rasterebeneneigenschaften\index{raster layers!properties} anzuzeigen. 
Abbildung \ref{fig:raster_properties} zeigt das Dialogfenster mit den
Rastereigenschaften. Es gibt fünf Reiter: \textsl{Bezeichnungen},
\textsl{Generell}, \textsl{Metadaten}, \textsl{Pyramiden} und \textsl{Histogramm}.
...
QGIS unterstützt drei Arten von Rasterebenen:\index{raster layers!supported channels}
...
Aus diesen drei Ebenentypen können acht Arten von Rastern dargestellt werden:
...
QGIS kann die Farbwerte einer Ebene invertieren, so dass helle Farben dunkel und
dunkle Farben hell werden. Benutzen Sie dazu die \textsl{Farben invertieren}
Checkbox, um diese Eigenschaft zu aktivieren oder zu deaktivieren.\index{raster layers!icolor map inversion}

QGIS kann Rasterebenen in unterschiedlicher Transparenz darstellen.
\index{raster layers!transparency} Benutzen Sie dazu den Transparenz Schieber,
um festzulegen, wieviel unter der aktuellen Rasterebene sichtbar sein soll.

% FIXME: Is this still correct? I cannot verify (SH)
QGIS kann so eingestellt werden, dass nur Rasterzellen gezeigt werden, die
Werte innerhalb einer definierten Standarbweichung zum Mittelwert der Ebene
...
\subsubsection{Generell}\label{label_generaltab}

Im Reiter Generell werden allgemeine Informationen zum ausgewählten Raster
...
muss die Checkbox \textit{Skalenabhängige Sichtbarkeit} aktiviert und ein
entsprechender Maßstab eingetragen werden.

Im Feld \textit{räumliches Referenzsystem} kann die Projektion abgelesen und mit
dem Knopf \textsl{Wechseln} verändert werden.

\subsubsection{Metadaten}\label{label_metatab}

Der Reiter Metadaten zeigt eine Anzahl von Informationen über das
Raster, inklusive Statistiken über jeden Kanal innerhalb der Ebene. Statistiken
werden nach dem Prinzip 'was brauche ich' erstellt. Es kann also gut sein, dass
für eine Rasterebene keine Statistik zur Verfügung steht oder gesammelt wurde.
\index{raster layers!metadata}

\begin{Tip}\caption{\textsc{Raster-Statistiken sammeln}}
\qgistip{Um statistische Daten zu einer Ebene zu sammeln, wählen Sie Pseudofarbe
und klicken auf \textsl{Anwenden}. Dieser Vorgang kann zeitaufwendig
...
Beachten Sie, dass das Bilden von Pyramiden die Originaldaten verändern und
...
Es gibt zwei Arten der Visualisierung: Balkendiagramm und Liniengrafik. 
