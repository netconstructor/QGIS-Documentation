...
\subsection{Que sont les données raster ?}\label{label_whatsraster}
...
\subsection{Charger des données raster dans QGIS}\label{label_loadraster}
...
données taster gérés par GDAL}.\index{couches raster!charger}
...
lacouche et choisissez \dropmenuopt{Propriétés} du menu
...
Quand vous sélectionnez \selectstring{couleurs indexées}{Colormap}, l'onglet
...
la moyenne pour la couche.\index{couches raster!déviation standard} Cela est
...
\qgistip{Si vous désirez visualiser une seule bande (par exemple la bande rouge)
d'une image multibande, vous pouvez penser que vous pourriez définir les bandes
Vertes et Bleue à ``Non définie''. Mais ce n'est pas la manière correcte. Pour
afficher la bande Rouge définissez le type d'image à nuance de gris, puis
sélectionnez Rouge comme bande à utiliser pour le Gris.
}
\end{Astuce} 
...
différents.\index{couches raster!transparence} Utiliser la barre coulissante de
...
une carte des reliefs ombrés superposé par une carte raster classifiée. Cela
rendra la carte encore plus proche de la 3e dimension.

% Additionally you can enter a rastervalue, which should be treated as
% {\em NODATA}.
De plus, vous pouvez entrer une valeur raster qui pourra être traité comme {\em
NODATA}
...
dans la section \guiheading{Options de transparence personnalisée}.
...
\filename{landcover.tif} à une transparence de 20 \%. Les étapes suivantes sont
nécessaire :
\begin{enumerate}
...
%  \dropmenuopt{Properties} from the popup meun.
 \item Ouvrez la boîte de dialogue \dialog{propriétées} en double-cliquant sur
...
Vous pouvez répéter les étapes \ref{enum:add} et \ref{enum:transp} pour ajuster
...
fichier} pour sauver vos paramètres  de transparence dans un fichier. Le
bouton \toolbtntwo{mActionAddRasterLayer}{Importer à partir d'un fichier} 
...
% \ref{label_sombology}).
L'onglet \tab{Colormap} est seulement disponible quand vous avez sélectionné un
rendu à une seule bande dans l'onglet  \tab{Sémiologie} (voir chapitre
\ref{label_sombology}).
...
Le bouton \button{Ajouter une entrée} ajoute une couleur à la table de couleur
...
dialogue \dialog{Sélectionner une couleur}  où vous pouvez sélectionner une
...
\toolbtntwo{mActionNewAttribute}{charger une carte de couleur à partir de
bande}  qui tente de charger la table à partir d'une bande (si celle-ci en a
une).
...
Le bloc \guiheading{Génèrer une nouvelle carte de couleurs} vous permet de
créer de nouvelles cartes de couleurs par catégorie. Vosu avez seulement besoin
de sélectionner le \selectnumber{nombre de classes}{15} dont vous avez besoin
et d'appuyez sur le bouton \button{Classifier}. Actuellement seule un 
\selectstring{mode de classification}{intervals égaux} est géré\index{couches
raster!classifier}.
...
symbol de la légende, et la palette.\index{couches raster!propriétés}
...
Cela peut être modifié en cliquant sur le bouton \button{Changer}.
...
statistique collectée.\index{couches raster!méta-données} 
...
\button{Rafraichir} en bas à droite, voir chapitre \ref{label_histogram}.
...
raster. Vous devez d'abord générer les statistiques du raster en cliquant le
bouton \button{Rafraichir}. Vous pouvez choisir quelles bandes à afficher en
les sélectionnant dans la liste déroulante en bas à gauche de l'onglet. Deux
types de graphiques différents sont permis :
...
Vous pouvez définir le nombre de colonnes du graphique à utiliser et décider si
...
remarquerez que les statistiques des bandes ont été remplies dans l'onglet
\tab{méta-données}.\index{couches raster!méta-données}
...
\qgistip{Rassembler des statistiques pour une couche, sélectionnez un rendu en
...
QGIS examine  vos données !\index{couches raster!statistiques}
}
\end{Astuce}
