...
Beenden, oder benutzen Sie die Tastenkombination \keystroke{Ctrl+Q}.
...
QGIS erstellt ein PNG-Bild mit 800x600 Pixeln. Sie können hinter der Option
\usertext{---snapshot} einen Dateinamen angeben.
...
\subsection{Die grafische Benutzeroberfläche (GUI)}\index{main window}
\label{label_qgismainwindow}

Beim Starten von QGIS öffnet sich die grafische Benutzeroberfläche (GUI) wie
in der folgenden Abbildung zu sehen. Die blauen Nummern von 1 bis 6 beziehen
...
abweichen in Abhängigkeit vom Betriebssystem und weiteren Einstellungen.
...
in der Werkzeugleiste zu finden ist und der Tastenkombination aufgelistet.
Obwohl fast jede Anwendung der Menüleiste ein entsprechendes Icon in der
Werkzeugleiste besitzt, ist die Anordnung unterschiedlich. Die Werkzeugleiste
mit der entsprechenden Anwendung ist hinter jedem Menüeintrag als
Kontrollkästchen angegeben. Weitere Informationen über die Werkzeuge in
...
\dropmenuopttwo{mActionZoomToSelected}{Zur Auswahl zoomen}
	\> \keystroke{Ctrl+J}
	\> \> \dropmenucheck{Kartennavigation} \\
\dropmenuopttwo{mActionZoomLast}{Zur vorherigen Zoomeinstellung zurückkehren}
	\>
	\> \> \dropmenucheck{Kartennavigation} \\
...
ein Layer angezeigt werden soll, oder nicht.\index{layer!visibility}
...
eines ausgewählten Layers als Übersicht dargestellt. Dadurch ist es sehr
einfach, einen schnellen Überblick über die genaue Position im Kartenfenster
zu bekommen. Beschriftungen werden nicht in der Kartenübersicht angezeigt.
...
auswählen können, on die Koordinaten des Bezugsystems oder der Ausdehnung des
...
das Kamera-Icon \toolbtntwo{mIconProjectionEnabled}{KBS Status} in der
unteren rechten Ecke der Statusleiste klicken. Dann wird die Maßeinheit
entsprechend der KBS eingestellt, z.B.: '+units=m'.
}
\end{Tip}

\subsection{Datenanzeige kontrollieren}\label{subsec:redraw_events}\index{rendering}
...
\minisec{a) Wiedergabe unterdrücken}\index{rendering!suspending}
...
\subsubsection{Strecken und Flächen messen}\index{measure:line length}\index{measure:areas}
...
Das Tool ermöglicht es, Punkte im Kartenfenster auszuwählen. Jedes
Längensegment wird in einem Popup angezeigt und zusätzlich auch die
Gesamtlänges aller Längensegmente. Um das Messen von Strecken zu beenden,
...
\subsection{QGIS Projekte}\label{sec:projects}\index{projects}
...
\subsection{Ausgabe}\label{sec:output}\index{output!save as image!print composer!quick print}
...
oder JPG-Format speichern können.
...
werden sollen}
\item \checkbox{Warung ausgeben, wenn QGIS-Projekt einer früheren Version
geöffnet wird}
...
\begin{itemize}
\item \checkbox{Standardmäßig werden alle neuen Layer im Kartenfenster
angezeigt}
...
Passen Sie die Optionen entsprechend Ihren Wünschen an. Einige Änderungen
können einen Neustart von QGIS notwendig machen, bevor sie aktiv werden.
