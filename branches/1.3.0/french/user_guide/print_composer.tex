...
\hline \includegraphics[width=0.7cm]{mActionSaveAsSVG} & Exporter la composition en SVG \\
...
cadre de carte de QGIS} dans la barre d'outils du composeur de carte et dessinez un rectangle dans le cadre du composeur avec le bouton gauche de la souris pour ajouter la carte. Vous verrez une boîte vide avec un message \textit{"La carte sera imprimée ici"}. Pour afficher la carte actuelle, choisissez
...
\begin{itemize}
\item \selectstring{Preview}{Rectangle} est le paramètre par défaut. Il n'affiche qu'une boîte vide avec le message \textit{"La carte sera imprimée ici"}. 
\item \selectstring{Preview}{Cache} affiche la carte dans sa résolution d'écran actuelle. Si vous zoomez sur la fenêtre de composition, la carte ne sera pas actualisée, mais l'image sera mise à l'échelle.
\item \selectstring{Preview}{Rendu} signifie que si vous faites un zoom dans la fenêtre de composition la carte sera actualisée, mais pour des raisons de performances, seulement jusqu'à une résolution maximum prédéfinie par QGIS.
\end{itemize}
...
% \subfigure[Properties dialog]
   \subfigure[Boîte de dialogue des propriétés]{\label{subfig:print_composer_map_item2}\includegraphics[clip=true,width=0.4\textwidth]{print_composer_map_item2}}
\end{figure}
...
Vous pouvez redimensionner la carte plus tard en cliquant sur le bouton \toolbtntwo{mActionSelectPan}{Sélectioner/Déplacer l'objet}, en sélectionnant
...
\item \toolbtntwo{mActionZoomOut}{Zoom -},
\item \toolbtntwo{mActionZoomFullExtent}{Zoomer sur l'emprise maximale} et
...
\subsubsection{Monter, descendre et aligner des éléments}
...
\subsubsection{Enregistrer et charger une mise en page d'impression}
